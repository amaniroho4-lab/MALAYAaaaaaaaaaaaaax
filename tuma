...
document.getElementById('dar').addEventListener('click',()=>{
  showWilaya("Dar es Salaam");
});

function showWilaya(region){
  // Clear old wilaya if exists
  let oldList = document.querySelector('.wilayaList');
  if(oldList) oldList.remove();

  // Show intro
  const intro = document.createElement('div');
  intro.className = 'intro';
  intro.textContent = `JE UNAHITAJI WAKALA WETU WA WILAYA GANI KATIKA MKOA HUU WA ${region.toUpperCase()}?`;
  regionList.appendChild(intro);

  // List of wilaya
  const wList = document.createElement('ul');
  wList.className = 'wilayaList';
  Object.keys(data[region]).forEach(w=>{
    const li = document.createElement('li');
    li.textContent = w;
    li.addEventListener('click',()=>showKata(region,w));
...
function showKata(region,wilaya){
  // Remove old kata list
  let oldKata = document.querySelector('.kataList');
  if(oldKata) oldKata.remove();
